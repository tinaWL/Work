...
SELECT e.personid, 'InfusionSoft_Email',
...
END) AS 'Email Template', p.firstname, NOW()
...
WHERE ea.ExternalActivityID IS NULL AND s.semesterid>=46 AND e.statusid =1 AND e.credithours > 0 and l.locationid in (1,2,3,4,5) and s.divisionid = 4 AND s.classid NOT IN(703, 704, 710, 711)  and l.locationname IN('Logan ILC', 'Pleasant Grove ILC')
group by e.personid;
...
-- parents
-- INSERT INTO externalactivity(`RecipientPersonID`, `ActivityType`, `ExternalID`, `CustomField01`, `CreatedDate`) 
SELECT pg.personid, 'InfusionSoft_Email',
...
END) AS 'Email Template', pg.firstname, NOW()
...
LEFT JOIN externalactivity ea ON ea.RecipientPersonID=pg.personid AND ea.ExternalID IN(13304,2222,13302)
WHERE ea.ExternalActivityID IS NULL AND s.semesterid>=46 AND e.statusid =1 AND e.credithours > 0 and l.locationid in (1,2,3,4,5) and s.divisionid = 4 AND s.classid NOT IN(703, 704, 710, 711)  and l.locationname IN('Logan ILC', 'Pleasant Grove ILC')
